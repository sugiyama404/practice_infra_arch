...
class PurchaseWorkflowService:
    ...
    def __init__(self):
        # データベース接続URL
        self.user_db_url = os.getenv('USER_DB_URL', 'mysql://user:user123@localhost:3306/user_service')
        self.payment_db_url = os.getenv('PAYMENT_DB_URL', 'mysql://payment:payment123@localhost:3307/payment_service')
        self.order_db_url = os.getenv('ORDER_DB_URL', 'mysql://order:order123@localhost:3308/order_service')
        self.saga_db_url = os.getenv('SAGA_DB_URL', 'mysql://saga:saga123@localhost:3309/saga_orchestrator')
    ...
    def create_purchase_workflow(self, purchase_request: Dict[str, Any]) -> str:
        ...
        logger.info(f"Creating purchase workflow for user {user_id}, product {product_id}, quantity {quantity}")
...
def interactive_demo():
    ...
    try:
        ...
        if status:
            ...
            print(f"  進行状況: {status['current_activity_index']}/{status['total_activities']}")
            print(f"  アクティビティ:")
