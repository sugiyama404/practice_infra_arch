...
class SagaWorkflowManager:
        ...
        try:
            ...
            while execution.current_activity_index < len(execution.activities):
                ...
                if success:
                    ...
                    logger.error(
                        f"Activity {activity.name} failed, starting compensation"
                    )
    ...
    def _execute_activity(
        self, execution: WorkflowExecution, activity: Activity
    ) -> bool:
        """
        ...
        for attempt in range(activity.max_retries + 1):
            try:
                ...
                if self._is_completable_error(e):
                    ...
                    logger.error(
                        f"Activity {activity.name} failed with completable error: {str(e)}"
                    )
                    ...
                    # 一時的エラーの場合、リトライ
                    logger.warning(
                        f"Activity {activity.name} failed (attempt {attempt + 1}), retrying: {str(e)}"
                    )
                    ...
                    # リトライ回数上限
                    logger.error(
                        f"Activity {activity.name} failed after {activity.max_retries + 1} attempts: {str(e)}"
                    )
                    activity.status = ActivityStatus.FAILED
                    return False
    ...
    def _compensate_workflow(self, execution: WorkflowExecution):
        ...
        for i in range(execution.current_activity_index - 1, -1, -1):
            ...
            if (
                activity.status == ActivityStatus.COMPLETED
                and activity.compensation_handler
            ):
                ...
                try:
                    ...
                    for attempt in range(max_compensation_retries):
                        try:
                            ...
                            logger.info(
                                f"Activity {activity.name} compensated successfully"
                            )
                            break
                        except Exception as e:
                            if attempt < max_compensation_retries - 1:
                                logger.warning(
                                    f"Compensation for {activity.name} failed (attempt {attempt + 1}), retrying: {str(e)}"
                                )
                                time.sleep(2**attempt)
                            else:
                                logger.error(
                                    f"Compensation for {activity.name} failed after {max_compensation_retries} attempts: {str(e)}"
                                )
                                raise
                    ...
                    execution.status = WorkflowStatus.FAILED
                    execution.error = (
                        f"Compensation failed for activity {activity.name}: {str(e)}"
                    )
                    self._save_workflow_execution(execution)
                    raise
    ...
    def _is_completable_error(self, error: Exception) -> bool:
        ...
        return any(
            completable_error in error_msg for completable_error in completable_errors
        )
