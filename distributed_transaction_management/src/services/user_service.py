...
class UserService:
    ...
    def __init__(self, db_url: str):
        ...
        self.users_table = Table(
            ...
            Column("reserved_balance", Numeric(10, 2), nullable=False, default=0.00),
            ...
            with self.SessionLocal() as session:
                ...
                if existing_reservation:
                    if existing_reservation.status == "reserved":
                        logger.info(
                            f"Balance already reserved for user {user_id}, transaction {transaction_id}"
                        )
                        return True
                    else:
                        raise Exception(
                            f"Reservation exists with status: {existing_reservation.status}"
                        )
                ...
                session.execute(
                    self.reservations_table.insert().values(**reservation_data)
                )
                ...
                session.commit()
                logger.info(
                    f"Reserved {amount} for user {user_id}, transaction {transaction_id}"
                )
                return True
        ...
        try:
            with self.SessionLocal() as session:
                ...
                if not reservation:
                    ...
                    confirmed = session.execute(
                        self.reservations_table.select().where(
                            (self.reservations_table.c.user_id == user_id)
                            & (
                                self.reservations_table.c.transaction_id
                                == transaction_id
                            )
                    ...
                    if confirmed:
                        logger.info(
                            f"Balance already confirmed for user {user_id}, transaction {transaction_id}"
                        )
                        return True
                ...
                session.execute(
                    ...
                    .where(self.reservations_table.c.id == reservation.id)
                    .values(status="confirmed")
                )

                session.commit()
                logger.info(
                    f"Confirmed {reservation.amount} for user {user_id}, transaction {transaction_id}"
                )
                return True
        ...
        try:
            with self.SessionLocal() as session:
                ...
                if not reservation:
                    ...
                    if cancelled:
                        logger.info(
                            f"Balance already cancelled for user {user_id}, transaction {transaction_id}"
                        )
                        return True
                ...
                session.commit()
                logger.info(
                    f"Cancelled {reservation.amount} for user {user_id}, transaction {transaction_id}"
                )
                return True
    ...
    def charge_payment(self, user_id: int, transaction_id: str) -> bool:
        ...
        try:
            with self.SessionLocal() as session:
                ...
                session.commit()
                logger.info(
                    f"Charged payment {payment_transaction.amount} for user {user_id}, transaction {transaction_id}"
                )
                return True
    ...
    def cancel_payment(self, user_id: int, transaction_id: str) -> bool:
        ...
        try:
            with self.SessionLocal() as session:
                ...
                session.commit()
                logger.info(
                    f"Cancelled payment {payment_transaction.amount} for user {user_id}, transaction {transaction_id}"
                )
                return True
