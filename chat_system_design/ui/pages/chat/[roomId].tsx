...
export default function ChatPage({ initialRoomId, userId, deviceId }: ChatPageProps) {
    ...
    const { messages, addMessage, setMessages, updateMessageStatus } = useChatStore();
    ...
    useEffect(() => {
            ...
            onEvent: (payload) => {
                switch (payload.event) {
                    ...
                    case 'ack':
                        ...
                        break;
                    case 'read':
                        if (payload.data?.message_id) updateMessageStatus(payload.data.message_id, 'read');
                        break;
                    case 'presence':
                    ...
                    case 'error':
                        if (typeof window !== 'undefined' && (window as any).UIkit?.notification) {
                            (window as any).UIkit.notification({
                                message: `Error: ${payload.data?.message || 'unknown'}`,
                                status: 'danger',
                            });
                        }
                        break;
        ...
        return () => client.close();
    }, [userId, deviceId, roomId, addMessage, setPresence, setTyping, updateMessageStatus]);
