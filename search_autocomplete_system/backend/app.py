...
DATABASE_URL = os.getenv(
    "DATABASE_URL", "mysql+pymysql://search_user:password123@localhost:3306/search_db"
)
...
class SearchTrie:
    ...
    def insert(self, term: str, category: str, popularity: int):
        ...
        node.term_data = {"term": term, "category": category, "popularity": popularity}
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(
                    "Redis接続に失敗しました。キャッシュ機能は無効になります。"
                )
                return None
...
def save_search_history():
    ...
    try:
        with engine.connect() as conn:
            ...
            result = conn.execute(
                check_sql, {"term": search_term, "session": user_session}
            )
            ...
            if existing:
                ...
                """)
                conn.execute(insert_sql, {"term": search_term, "session": user_session})
