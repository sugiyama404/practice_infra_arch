...
def get_redis_nodes():
    redis_nodes_str = os.environ.get("REDIS_NODES", "redis-node1:6379,redis-node2:6379,redis-node3:6379")
    nodes = []
    for node_addr in redis_nodes_str.split(','):
        host, port = node_addr.split(':')
        nodes.append({"name": f"node{len(nodes)+1}", "host": host, "master_port": int(port), "slave_port": int(port)})
    return nodes

NODES = get_redis_nodes()
nodes = {n['name']: ShardNode(n['name'], n['host'], n['master_port'], n['slave_port']) for n in NODES}
