...
| 要素           | サービス                                             |
| ------------ | ------------------------------------------------ |
| 学習環境         | ローカル環境でh5ファイル作成 + S3アップロード                      |
| 操作画面         | ECS Fargate + ALB（シンプルなWebインターフェース）              |
| トレーディング実行環境  | Lambda + API Gateway HTTP API                   |
| API          | API Gateway HTTP API（RESTfulより低コスト・シンプル）          |
| ストレージ        | S3（データ/モデル/h5ファイル保存）                            |
| ログ監視         | CloudWatch                                       |
