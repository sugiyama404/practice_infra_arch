...
CREATE TABLE saga_instances (
    ...
    saga_type VARCHAR(50) NOT NULL DEFAULT 'ORDER_PROCESSING',
    order_id VARCHAR(50) NOT NULL,
    status ENUM('STARTED', 'ORDER_CREATED', 'STOCK_RESERVED', 'PAYMENT_COMPLETED', 'SHIPPING_ARRANGED', 'COMPLETED', 'COMPENSATION_STARTED', 'FAILED') NOT NULL DEFAULT 'STARTED',
...
CREATE TABLE saga_step_logs (
    ...
    status ENUM('STARTED', 'COMPLETED', 'FAILED', 'COMPENSATED') NOT NULL,
    ...
    INDEX idx_saga_step_logs_saga_id (saga_id),
    INDEX idx_saga_step_logs_status (status)
);
